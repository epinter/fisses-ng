...
namespace Fisses {
    ...
    RE::BSFixedString CFissBeginLoad([[maybe_unused]] RE::StaticFunctionTag* base, RE::BSFixedString filename) {
        ...
        if (enc_name != NULL)
            cfiss_filename_encode(filename.data(), enc_name, enc_name_size);
    ...
    RE::BSFixedString CFissLoadString([[maybe_unused]] RE::StaticFunctionTag* base, RE::BSFixedString obj,
    ...
    // ===================================================================================================================================================
    RE::BSFixedString CFissBeginSave([[maybe_unused]] RE::StaticFunctionTag* base, RE::BSFixedString filename,
                                     RE::BSFixedString modname) {
        CFISS* obj = (CFISS*)calloc(1, sizeof(CFISS));
        unsigned int enc_name_size = cfiss_filename_encode(filename.data(), NULL, 0) + 8;
        char* enc_name = (char*)malloc(enc_name_size);
        if (enc_name != NULL)
        ...
        logger::debug("cfiss_beginSave({}, {}) return {}", (enc_name == NULL) ? filename.data() : enc_name, modname.data(),
                      fmt::ptr(obj));
        if (enc_name != NULL)
            free(enc_name);
    ...
    void CFissSaveString([[maybe_unused]] RE::StaticFunctionTag* base, RE::BSFixedString obj, RE::BSFixedString name,
                         RE::BSFixedString value) {
        CFISS* cobj = CFissObjectUnpack(obj.data());
        char* data = cfiss_xml_encoder(value.data());
        cfiss_saveString(cobj, name.data(), data);

        logger::debug("({})->cfiss_saveString({}, {})", fmt::ptr(cobj), name.data(), data);
        if (data != value.data())
            free(data);
    }
    void CFissSaveFloat([[maybe_unused]] RE::StaticFunctionTag* base, RE::BSFixedString obj, RE::BSFixedString name,
                        float value) {
        CFISS* cobj = CFissObjectUnpack(obj.data());
    ...
    RE::BSFixedString CFissSaveTextToTxtFile([[maybe_unused]] RE::StaticFunctionTag* base, RE::BSFixedString filename,
