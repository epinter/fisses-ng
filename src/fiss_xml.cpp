...
static bool is_valid_character(char ch) {
    ...
    if (0x01 <= uch && uch <= 0x1F)
        return false;
    else if (0x7F <= uch && uch <= 0xFF)
        return false;
...
void cfiss_xml_decoder(char* str) {
    ...
    for (i = 0, j = 0; src[j] != 0;) {
        if (src[j] == '&') {
            if (strncmp(&src[j], "&lt;", 4) == 0) {
                dst[i] = '<';
                j += 4;
            } else if (strncmp(&src[j], "&gt;", 4) == 0) {
                dst[i] = '>';
                j += 4;
            } else if (strncmp(&src[j], "&amp;", 5) == 0) {
                dst[i] = '&';
                j += 5;
            } else if (strncmp(&src[j], "&apos;", 6) == 0) {
                dst[i] = '\'';
                j += 6;
            } else if (strncmp(&src[j], "&quot;", 6) == 0) {
                dst[i] = '\"';
